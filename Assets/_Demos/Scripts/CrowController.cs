...
namespace BlueBird.Demos
{
    public class CrowController : MonoBehaviour
    {
        ...
        private void MovePosition()
        {
            ...
            // 计算物体的仰角幅度：向上飞行，速度会下降；向下飞行，速度会提升。
            // 世界坐标系中，“世界上方向”同“物体前方向”的cos夹角。
            //  没有使用“世界前方向”，因为其对上仰、下潜的cos夹角值符号相同，无法做出区分。
            float elevation = Vector3.Dot(transform.forward, Vector3.up);
            // 夹角小于90度，向上飞行
            if (elevation > 0.0f)
            {
                // 速度减小，越接近水平，减弱幅度越强
                // 函数为：值域[0.5, 1.0]，且斜率（随夹角增大）逐渐增大的函数
                speed *= (0.5f + 0.5f * (1.0f - elevation));
            }
            // 夹角大于90度，向下飞行
            else if (elevation < 0.0f)
            {
                // 速度增加，越接近水平，减弱幅度越强
                // 函数为：值域[1.0, 1.5]，且斜率（随夹角增大）逐渐减小的函数
                speed *= (1.0f - 0.5f * elevation);
            }
